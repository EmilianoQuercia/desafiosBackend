                ...
                fs.readFile(this.path,(error,data)=>{
                    ...
                    for (const key in prod) {
                        if (prod[key]=== '' || !prod[key] ){
                                console.log(`Al Producto ingresado le falto completar un campo`);
                                return resolve({warning:`Al Producto ingresado le falto completar un campo ${key}`})
                        }  
                        contador += 1  
                    }

                    if ( contador < 8){
                        return resolve({warning:'Al Producto ingresado le falto completar un campo'})
                    }
                    ...
                    if (cod === undefined){
                        ...
                        resolve({warning:`Su Producto ${prod.title}, tiene un codigo que ya existe: ${prod.code}`})
                        return
                    }
        ...
        return new Promise((resolve, reject)=>{
            fs.readFile(this.path,'utf-8',(error,data)=>{
                ...
                if (productId) {
                        ...
                        resolve({error: 'Product by Id: NOT FOUND'});
                    }
            })
        })
    }
